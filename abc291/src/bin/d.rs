...
fn main() {
    ...
    let mut dp = (ModInt::new(1), ModInt::new(1));
    for (i, (a, b)) in ab.iter().copied().enumerate().skip(1) {
        let (da, db) = dp;
        let (pa, pb) = ab[i - 1];
        let (mut na, mut nb) = (ModInt::new(0), ModInt::new(0));
...
pub mod modint {
    ...
    mod tests {
        ...
        fn x() {
            let mut dp = vec![1; 3];
            dp[0] += dp[1];
            assert_eq!(dp, vec![2, 1, 1]);

            let mut dp = vec![1_f64; 3];
            dp[0] += dp[1];
            assert_eq!(dp, vec![2_f64, 1_f64, 1_f64]);

            let mut dp = vec![ModInt1000000007::new(1); 3];
            dp[0] += dp[1];
            assert_eq!(
                dp,
                vec![
                    ModInt1000000007::new(2),
