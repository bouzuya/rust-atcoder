...
fn dfs(edges: &[Vec<(usize, usize)>], weight: &mut Vec<i64>, u: usize, p: usize, g: usize) -> bool {
    if u == g {
        return true;
    }
...
fn main() {
    ...
    let mut weight = vec![0_i64; n - 1];
    let mut u = a[0];
    for v in a.into_iter().skip(1) {
        dfs(&edges, &mut weight, u, u, v);
        u = v;
    }

    let sum = weight.iter().sum::<i64>();
    let mut dp = HashMap::new();
    dp.insert(0, ModInt::new(1));
    for w in weight.iter().copied() {
        let mut next = dp.clone();
        for (d, c) in dp {
            *next.entry(d + w).or_insert(ModInt::new(0)) += c;
        }
        dp = next;
    }
    ...
    let ans = *dp.get(&((k + sum) / 2)).unwrap_or(&ModInt::new(0));
    println!("{}", ans);
}
