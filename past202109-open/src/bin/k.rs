...
fn main() {
    ...
    let v_t = p + q + 1;
    for v in 0..p {
        mcf.add_edge(v_s, v, 1, 0);
    }
    for u in p..p + q {
        mcf.add_edge(u, v_t, 1, 0);
    }
    let x = 1_i64 << 32;
    for (u, (a_u, b_u)) in ab.iter().copied().enumerate() {
        for (v, (c_v, d_v)) in cd.iter().copied().enumerate() {
            if s[u][v] == '0' {
                continue;
            }
            let w = a_u + c_v - b_u - d_v;
            if w <= 0 {
                continue;
            }
            mcf.add_edge(u, p + v, 1, x - w);
        }
    }
    let base = ab.iter().copied().map(|(_, b)| b).sum::<i64>()
        + cd.iter().copied().map(|(_, d)| d).sum::<i64>();
    let ans = mcf
        .slope(v_s, v_t, i64::max_value())
        .into_iter()
        .map(|(flow, min_cost)| base + x * flow - min_cost)
        .max()
        .unwrap()
        .max(base);
