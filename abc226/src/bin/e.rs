...
fn main() {
    ...
    let mut count_component_edge = vec![0; n];
    for (u, _) in uv.iter().copied() {
        count_component_edge[dsu.leader(u)] += 1;
    }

    let mut ans = ModInt::new(1);
    for group in dsu.groups() {
        let count_component_vertex = group.len();
        if count_component_vertex != count_component_edge[dsu.leader(group[0])] {
            println!("0");
            return;
        }
        ans *= ModInt::new(2);
