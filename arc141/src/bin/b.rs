...
fn main() {
    ...
    let l = format!("{:b}", m).len();
    let mut count = vec![0; l + 1];
    for i in 1..=l {
        let max = 2_usize.pow(i as u32).saturating_sub(1).min(m);
        let min = 2_usize.pow((i - 1) as u32);
        count[i] = max.saturating_sub(min.saturating_sub(1));
    }

    let mut dp = vec![ModInt::new(0); l + 1];
    dp[0] = ModInt::new(1);
    for _ in 0..n {
        let mut next = vec![ModInt::new(0); l + 1];
        for j in 1..=l {
            for k in 0..j {
                next[j] += dp[k] * count[j];
            }
