...
fn main() {
    input! {
        w: usize,
        n: usize,
        lrv: [(Usize1, Usize1, i64); n],
    };
    let max_w = 10_000;
    let mut dp: Vec<Segtree<Max<i64>>> = vec![];
    for _ in 0..=n {
        dp.push(Segtree::new(max_w + 1));
    }
    dp[0].set(0, 0);
    for (i, &(l_i, r_i, v_i)) in lrv.iter().enumerate() {
        for j in 0..=max_w {
            let x = dp[i].get(j);
            if x > dp[i + 1].get(j) {
                dp[i + 1].set(j, x);
            ...
            if j >= l_i && j > 0 {
                let x = dp[i].prod((j - 1).saturating_sub(r_i), j - l_i);
                if x >= 0 && x + v_i > dp[i + 1].get(j) {
                    dp[i + 1].set(j, x + v_i);
                }
            }
        }
