...
fn dfs(n: usize, c: &[char], edges: &[Vec<usize>], dp: &mut [Vec<ModInt>], u: usize, p: usize) {
    ...
    for v in edges[u].iter().copied() {
        ...
        dfs(n, c, edges, dp, v, u);
        a_only *= dp[v][0] + dp[v][2];
        b_only *= dp[v][1] + dp[v][2];
        all *= dp[v][0] + dp[v][1] + two * dp[v][2];
    }
    let a_only = if c[u] == 'a' { a_only } else { zero };
    let b_only = if c[u] == 'b' { b_only } else { zero };
    dp[u][0] = a_only;
    dp[u][1] = b_only;
    dp[u][2] = all - a_only - b_only;
}
...
fn main() {
    ...
    // dp[i][j] の部分木において j = 0: a only, 1: b only, 2: both の場合の数
    let mut dp = vec![vec![ModInt::new(0); 3]; n];
    dfs(n, &c, &edges, &mut dp, 0, 0);
