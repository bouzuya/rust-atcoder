...
fn main() {
    ...
    hwd.sort_by_key(|it| it[0]);

    let wmap = std::iter::once(0)
        .chain(hwd.iter().map(|v| v[1]))
        .collect::<BTreeSet<_>>()
        .into_iter()
        .enumerate()
    ...
    let mut prev = vec![(hwd[0][0], hwd[0][1], hwd[0][2])];
    let mut wd = Segtree::<Min<usize>>::new(wmap.len());
    for it in hwd.iter() {
        let (h, w, d) = (it[0], it[1], it[2]);
        if prev[0].0 < h {
            for (_, w1, d1) in prev.iter().copied() {
                wd.set(wmap[&w1], wd.get(wmap[&w1]).min(d1));
            }
            prev.clear();
        }
        prev.push((h, w, d));
        let min_d = wd.prod(wmap[&0], wmap[&w]);
        if d > min_d {
            println!("Yes");
            return;
        }
