...
fn main() {
    ...
    let mut dp = vec![vec![ModInt::new(0); 2]; 2];
    dp[if y1 == y2 { 0 } else { 1 }][if x1 == x2 { 0 } else { 1 }] = ModInt::new(1);
    for _ in 0..k {
        let mut next = vec![vec![ModInt::new(0); 2]; 2];
        next[0][0] = dp[0][1] + dp[1][0];
        next[0][1] = ModInt::new(h - 1) * dp[0][0] + ModInt::new(h - 2) * dp[0][1] + dp[1][1];
        next[1][0] = ModInt::new(w - 1) * dp[0][0] + ModInt::new(w - 2) * dp[1][0] + dp[1][1];
        next[1][1] = ModInt::new(w - 1) * dp[0][1]
            + ModInt::new(h - 1) * dp[1][0]
            + ModInt::new(w - 2 + h - 2) * dp[1][1];
        dp = next;
    }
    let ans = dp[0][0];
    println!("{}", ans);
