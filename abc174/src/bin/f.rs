...
fn main() {
    ...
    // Vec<(s, t)> t と同じ色の一つ手前の石の位置 s
    let st = {
        let mut e = vec![];
        let mut p = vec![n; n];
        for (i, &c_i) in c.iter().enumerate().rev() {
            if p[c_i] != n {
                e.push((i, p[c_i]));
            }
            p[c_i] = i;
    ...
    // 答えは各 (l, r) について (r - l + 1) - (範囲内の石の重複数)
    // 範囲内の石の重複数は l, r に含まれる s, t の個数と同じ
    // 範囲内かの判定は l <= s && t <= r
    // l <= s および t <= r は独立に判定可能
    // l, s を x 軸, r, t を y 軸にとった二次元平面において
    // l, r の右下にある点の個数が範囲内の石の重複数になる
    // x の降順に走査して y 以下の点の個数を数える
    let mut p = vec![];
    ...
    p.sort_by_key(|&(t, x, _, _)| (-(x as i64), t));

    let mut d = vec![n + 1; q];
    let mut segtree = Segtree::<Additive<_>>::new(n);
    for (t_i, _, y_i, i) in p {
        match t_i {
            1 => {
                // (s, t)
                segtree.set(y_i, 1);
