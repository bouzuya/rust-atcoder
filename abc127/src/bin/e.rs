...
fn main() {
    ...
    let mut fact = vec![ModInt::new(0); n * m + 1];
    fact[0] = ModInt::new(1);
    for i in 1..=n * m {
        fact[i] = fact[i - 1] * ModInt::new(i);
    }
    let mut finv = vec![ModInt::new(0); n * m + 1];
    finv[n * m] = fact[n * m].inv();
    for i in (1..=n * m).rev() {
        finv[i - 1] = finv[i] * ModInt::new(i);
    }
    let nck = |n, k| fact[n] * finv[k] * finv[n - k];

    let mut ans = ModInt::new(0);
    for d in 1..=n {
        ans += ModInt::new(d) * nck(n * m - 2, k - 2) * (n - d) * m * m;
    }
    for d in 1..=m {
        ans += ModInt::new(d) * nck(n * m - 2, k - 2) * (m - d) * n * n;
    }
    println!("{}", ans);
}
