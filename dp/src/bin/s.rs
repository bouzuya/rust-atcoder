...
fn main() {
    ...
    let mut dp = vec![vec![zero; d]; 2];
    dp[0][0] = one;
    for k_i in k {
        let mut next = vec![vec![zero; d]; 2];
        ...
        for j in 0..d {
            for digit in 0..=9 {
                ...
                next[1][l] += dp[1][j];
                match digit.cmp(&k_i) {
                    std::cmp::Ordering::Less => next[1][l] += dp[0][j],
                    std::cmp::Ordering::Equal => next[0][l] += dp[0][j],
