...
fn main() {
    ...
    let mut dp = vec![ModInt::new(0); capital_k + 1];
    dp[0] = ModInt::new(1);
    for a_i in a.iter().copied() {
        let mut next = vec![ModInt::new(0); capital_k + 1];
        let c = std::iter::once(ModInt::new(0))
            .chain(dp.iter().scan(ModInt::new(0), |acc, &i| {
                *acc += i;
                Some(*acc)
            }))
            .collect::<Vec<ModInt>>();
        for j in 0..=capital_k {
            // for k in 0..=a_i {
            //     if j + k <= capital_k {
            //         next[j + k] += dp[j];
            //     }
            // }
            next[j] = c[j + 1] - if j >= a_i { c[j - a_i] } else { ModInt::new(0) };
        }
        dp = next;
    }
    let ans = dp[capital_k];
    println!("{}", ans);
}
