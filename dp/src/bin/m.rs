// <https://blog.hamayanhamayan.com/entry/2019/01/10/235150>
use fenwicktree::*;
...
fn main() {
    ...
    let mut dp = vec![];
    for _ in 0..=n {
        dp.push(FenwickTree::new(k + 1, ModInt::new(0)));
    }

    dp[0].add(k, ModInt::new(1));
    for i in 0..n {
        for j in 0..=k {
            let v = dp[i].sum(j, k.min(j + a[i]) + 1);
            dp[i + 1].add(j, v);
        }
    }

    let ans = dp[n].sum(0, 1);
    println!("{}", ans);
}
