...
fn dfs(e: &[Vec<usize>], dp: &mut [(ModInt, ModInt)], u: usize, p: usize) {
    let mut w = ModInt::new(1); // if e[u].len() == 1 { 1 } else { 0 });
    let mut b = ModInt::new(1); // if e[u].len() == 1 { 1 } else { 0 });
...
fn main() {
    ...
    let mut dp = vec![(ModInt::new(0), ModInt::new(0)); n];
    dfs(&e, &mut dp, 0, 0);
