...
fn main() {
    ...
    impl MapMonoid for F {
        type M = M;
        type F = Option<ModInt>;
        fn identity_map() -> Self::F {
            None
        }
        fn mapping(&f: &Self::F, &x: &<Self::M as Monoid>::S) -> <Self::M as Monoid>::S {
            match f {
                Some(d) => (d * (x.1 - ModInt::new(1)) * ModInt::new(9).inv(), x.1),
                None => x,
            }
        }
    ...
    for i in 0..n {
        lazysegtree.set(i, (ModInt::new(1), ModInt::new(10)));
    }
    for (l, r, d) in lrd {
        lazysegtree.apply_range(l - 1..r, Some(ModInt::new(d)));
        println!("{}", lazysegtree.prod(0..n).0);
    }
}
