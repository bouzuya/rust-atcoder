...
fn main() {
    ...
    let s_len = s.len();
    let t_len = s.len() + k;
    let mc = ModCombin::new(t_len + 1);
    let mut ans = ModU32::new(0);
    for p_s_n in s_len..=t_len {
        let u_len = t_len - p_s_n; // Sn 以降の文字列の長さ
        ans += ModU32::new(25).pow((k - u_len) as u32)
            * ModU32::new(26).pow(u_len as u32)
            * mc.combination(t_len - u_len - 1, s_len - 1);
    }
    println!("{}", ans);
}
