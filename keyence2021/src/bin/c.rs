...
fn main() {
    ...
    let mut b = vec![vec![Dir::B; w]; h];
    for (h_i, w_i, c_i) in hwc {
        b[h_i][w_i] = match c_i {
            'R' => Dir::R,
            'D' => Dir::D,
            'X' => Dir::X,
    ...
    let inv = ModInt::new(2) / ModInt::new(3);
    ...
    for i in 0..h {
        for j in 0..w {
            match b[i][j] {
                ...
                Dir::B => {
                    if j + 1 < w {
                        dp[i][j + 1] = dp[i][j + 1] + (dp[i][j] * inv);
                    }
                    if i + 1 < h {
                        dp[i + 1][j] = dp[i + 1][j] + (dp[i][j] * inv);
                    }
                }
                _ => unreachable!(),
            }
    ...
    let ans = dp[h - 1][w - 1] * ModInt::new(3).pow((h * w - k) as u64);
    println!("{}", ans);
}
