...
fn main() {
    ...
    let mut dp = vec![ModInt::new(0); 3_000 + 2];
    for j in a[0]..=b[0] {
        dp[j] = ModInt::new(1);
    }
    for i in 1..n {
        let mut next = vec![ModInt::new(0); 3_000 + 2];
        for j in 0..=3_000 {
            if j > b[i] {
                continue;
            }

            // a[i].max(j) から b[i] の区間に dp[j] を加算する
        ...
        for j in 0..3_000 {
            next[j + 1] = next[j + 1] + next[j]
        }

        dp = next;
    }
    let ans = dp.iter().copied().take(3_000 + 1).sum::<ModInt>();
    println!("{}", ans);
}
