...
fn main() {
    ...
    if m as u64 % u64::from(ModInt::modulus()) == 0 {
        println!("0");
        return;
    }

    modint::DynamicModInt::<DefaultId>::set_modulus(ModInt::modulus() - 1);
    let r = modint::DynamicModInt::<DefaultId>::new(k)
        .pow(n as u64)
        .val();
    let ans = ModInt::new(m).pow(r as u64);
    println!("{}", ans);
}
