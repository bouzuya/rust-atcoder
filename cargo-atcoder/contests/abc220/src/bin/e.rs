...
fn main() {
    ...
    let m0 = ModInt::new(0);
    let m2 = ModInt::new(2);
    let mut pow = vec![]; // pow[i] = 2^i
    for x in 0..=2 * n as u64 {
        pow.push(m2.pow(x));
    }

    let mut ans = m0;
    for cur_depth in 1..=n {
        if cur_depth + dist <= depth {
            ans += pow[dist] * pow[cur_depth];
        }
        let l = cmp::max(1, (cur_depth + dist).saturating_sub(depth));
        let r = cmp::min(dist, depth.saturating_sub(cur_depth - 1));
        if r > l && dist >= 2 {
            ans += ModInt::new(r - l) * pow[dist - 2] * pow[cur_depth];
        }
    }
