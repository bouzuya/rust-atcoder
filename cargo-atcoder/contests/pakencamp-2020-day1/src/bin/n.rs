// <https://drken1215.hatenablog.com/entry/2021/01/06/131400>
...
fn main() {
    ...
    a.push_back(n + 1);
    a.push_back(0);
    let mut st = Segtree::<Min<_>>::new(n + 2);
    st.set(0, a[1]);
    for i in 1..=n + 1 {
        let mut x = st.prod(0, a[i]) + a[i - 1] + a[i + 1];
        if i > 0 && a[i - 1] < a[i] {
            x = x.min(st.prod(a[i - 1], a[i - 1] + 1) - a[i] + a[i + 1]);
        }
        st.set(a[i], x);
    }
    let ans = st.prod(n + 1, n + 2);
    println!("{}", ans);
}
