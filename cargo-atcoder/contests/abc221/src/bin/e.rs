...
fn main() {
    ...
    for (i, x) in set.iter().copied().enumerate() {
        map.insert(x, i);
    }

    let mut bit = FenwickTree::new(map.len(), ModInt::new(0));
    let mut ans = ModInt::new(0);
    for (i, a_i) in a.iter().copied().rev().enumerate() {
        let index = *map.get(&a_i).unwrap();
        ans += bit.sum(index, map.len()) * ModInt::new(2).pow(i as u64);
        bit.add(index, (ModInt::new(1) / ModInt::new(2)).pow((i + 1) as u64));
    }
