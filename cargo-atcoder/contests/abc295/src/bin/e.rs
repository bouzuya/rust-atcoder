...
fn main() {
    ...
    let mut bit = FenwickTree::new(max_a + 1, 0);
    for a_i in a.iter().copied() {
        bit.add(a_i, 1);
    }

    // l..=r
    let count = |l: usize, r: usize| -> usize { bit.sum(l, r + 1) };
    ...
    let mut f = vec![ModInt::new(0); m + 1];
    let count_z = count(0, 0);
    for i in 0..=m {
        let count_lte = count(1, i);
        for j in 0..=count_z {
            if count_lte + j < k {
                let mut x = binom(count_z, j);
                x *= ModInt::new(i).pow(j as u64);
                x *= ModInt::new(m - i).pow((count_z - j) as u64);
                f[i] += x;
            }
        }
    }
    ...
    ans /= ModInt::new(m).pow(count_z as u64);
