...
use segtree::*;
struct MaxF;
impl MapMonoid for MaxF {
    type M = Max<usize>;
    type F = usize; // |x| max(v, x) だが max は持たず v のみを保持する
    ...
    fn mapping(f: &Self::F, x: &<Self::M as Monoid>::S) -> <Self::M as Monoid>::S {
        cmp::max(f.clone(), x.clone())
    }

    fn composition(f: &Self::F, g: &Self::F) -> Self::F {
        cmp::max(f.clone(), g.clone())
    }
}
...
fn main() {
    ...
    let mut segtree: LazySegtree<MaxF> = LazySegtree::new(w);
    for (l_i, r_i) in lr {
        let v = segtree.prod(l_i, r_i + 1) + 1;
        segtree.apply_range(l_i, r_i + 1, v);
        println!("{}", v);
    }
}
