...
fn main() {
    ...
    let mut count_a = vec![0_usize; n + 1];
    let mut count_ql = vec![0_usize; n + 1];
    for (i, s_i) in s.iter().copied().enumerate() {
        count_a[i + 1] = count_a[i] + if s_i == 'A' { 1 } else { 0 };
        count_ql[i + 1] = count_ql[i] + if s_i == '?' { 1 } else { 0 };
    }
    let mut count_c = vec![0_usize; n + 1];
    let mut count_qr = vec![0_usize; n + 1];
    for (i, s_i) in s.iter().copied().enumerate().rev() {
        count_c[i] = count_c[i + 1] + if s_i == 'C' { 1 } else { 0 };
        count_qr[i] = count_qr[i + 1] + if s_i == '?' { 1 } else { 0 };
    }
    ...
    for i in 0..n {
        ...
        let c = count_c[i + 1];
        let qr = count_qr[i + 1];
        let l = ModInt::new(a) * ModInt::new(3).pow(ql as u64)
            + ModInt::new(ql) * ModInt::new(3).pow(ql.saturating_sub(1) as u64);
        let r = ModInt::new(c) * ModInt::new(3).pow(qr as u64)
            + ModInt::new(qr) * ModInt::new(3).pow(qr.saturating_sub(1) as u64);
        let x = l * r;
