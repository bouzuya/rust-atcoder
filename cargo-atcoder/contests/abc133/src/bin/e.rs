...
fn dfs<F>(res: &mut ModInt, e: &Vec<Vec<usize>>, k: usize, permutation: &F, u: usize, p: usize)
where
    F: Fn(usize, usize) -> ModInt,
{
    if u == 0 {
        let c = e[u].len();
        if c == 0 {
        ...
        *res *= permutation(k - 2, c);
    }
    for &v in e[u].iter() {
        if v == p {
            continue;
        }
        dfs(res, e, k, permutation, v, u);
    }
}
...
fn main() {
    ...
    let max_n = cmp::max(n, k);
    let mut fact = vec![ModInt::new(0); max_n + 1];
    fact[0] = ModInt::new(1);
    for i in 1..=max_n {
        fact[i] = fact[i - 1] * ModInt::new(i);
    }
    let mut finv = vec![ModInt::new(0); max_n + 1];
    finv[max_n] = fact[max_n].inv();
    for i in (1..=max_n).rev() {
        finv[i - 1] = finv[i] * ModInt::new(i);
    }
    let permutation = |n: usize, k: usize| -> ModInt {
        if n < k {
            return ModInt::new(0);
        }
        fact[n] * finv[n - k]
    ...
    let mut res = ModInt::new(k);
    dfs(&mut res, &e, k, &permutation, 0, 0);
    let ans = res;
    println!("{}", ans);
}
