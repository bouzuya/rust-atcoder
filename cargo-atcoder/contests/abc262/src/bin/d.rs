...
fn main() {
    ...
    let mut ans = ModInt::new(n);
    for x in 2..=n {
        // x 項選ぶとき、それらの和が x で割り切れる場合の数を ans に加算する
        let mut dp = vec![vec![ModInt::new(0); x]; x + 1];
        dp[0][0] = ModInt::new(1);
        for a_i in a.iter().copied() {
            let a_i = a_i % x;
            let mut next = vec![vec![ModInt::new(0); x]; x + 1];
            for i in 0..=x {
                for j in 0..x {
                    // a_i を選ばないとき
                    next[i][j] += dp[i][j];
