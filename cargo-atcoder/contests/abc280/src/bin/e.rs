...
fn f(dp: &mut Vec<Option<ModInt>>, p: usize, x: usize) -> ModInt {
    if dp[x].is_some() {
        return dp[x].unwrap();
    }
    let p1 = ModInt::new(p) / ModInt::new(100);
    let p2 = ModInt::new(1) - p1;
    if x == 0 {
        let v = ModInt::new(0);
        dp[x] = Some(v);
        v
        ...
        let v = p1 * f(dp, p, x - 2) + p2 * f(dp, p, x - 1) + ModInt::new(1);
        dp[x] = Some(v);
        v
    }
}
