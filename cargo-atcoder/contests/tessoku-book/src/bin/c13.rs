...
fn main() {
    ...
    let a = a.into_iter().map(|a_i| a_i % m).collect::<Vec<usize>>();

    if p == 0 {
        let count_zero = a.iter().filter(|a_i| a_i == &&0).count();
        let ans = (count_zero * (n - 1) + (n - count_zero) * count_zero) / 2;
        println!("{}", ans);
        return;
    } else if p == 1 {
        let count_one = a.iter().filter(|a_i| a_i == &&1).count();
        let ans = count_one * (count_one - 1) / 2;
        println!("{}", ans);
        return;
    }
    ...
    let mut ans = 0_usize;
    for (&a, &count_a) in count.iter() {
        let b = (ModInt::new(p) / ModInt::new(a)).val() as usize;
        let count_b = if a == b {
            count_a - 1
        } else {
            *count.get(&b).unwrap_or(&0)
        };
        ans += count_a * count_b;
    }
    ans /= 2;
