...
fn main() {
    ...
    if k == 0 {
        let ans = ModInt::new(m).pow(n as u64);
        println!("{}", ans);
        return;
    }
    let mut dp = FenwickTree::new(m + 1, ModInt::new(0));
    for i in 1..=m {
        dp.add(i, ModInt::new(1));
    }
    for _ in 1..n {
        let mut next = FenwickTree::new(m + 1, ModInt::new(0));
        for j in 1..=m {
            if j > k {
                next.add(j, dp.sum(1, j - k + 1));
            }
            if j + k <= m {
                next.add(j, dp.sum(j + k, m + 1));
            }
        }
        dp = next;
    }
    let ans = dp.sum(1, m + 1);
    println!("{}", ans);
}
