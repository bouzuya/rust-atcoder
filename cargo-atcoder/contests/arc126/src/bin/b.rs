...
fn main() {
    ...
    for (a_i, b_i) in ab {
        map.entry(a_i).or_insert_with(Vec::new).push(b_i);
    }

    let mut dp = Segtree::<Max<usize>>::new(n);
    for (_, b_i) in map.iter_mut() {
        b_i.sort();
        let mut xs = vec![];
        for b_ij in b_i.iter().copied() {
            let x = dp.prod(0, b_ij);
            xs.push((b_ij, x + 1));
        }
    ...
    let ans = (0..n).map(|i| dp.get(i)).max().unwrap();
    println!("{}", ans);
}
