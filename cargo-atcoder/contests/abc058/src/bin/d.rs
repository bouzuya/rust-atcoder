...
fn main() {
    ...
    let mut x_sum = ModU32::new(0);
    for (i, &x_i) in x.iter().enumerate() {
        let j = (i + 1) as i32;
        x_sum = x_sum + ModU32::from(j - 1) * ModU32::from(x_i)
            - ModU32::from(n - j) * ModU32::from(x_i);
    }
    let mut y_sum = ModU32::new(0);
    for (i, &y_i) in y.iter().enumerate() {
        let j = (i + 1) as i32;
        y_sum = y_sum + ModU32::from(j - 1) * ModU32::from(y_i)
            - ModU32::from(m - j) * ModU32::from(y_i);
    }
    let ans = x_sum * y_sum;
    println!("{}", ans);
}
    ...
    impl From<i32> for ModU32 {
        fn from(x: i32) -> Self {
            if x >= 0 {
                ...
                ModU32::new(0) - ModU32::new(-x as u64)
            }
        }
    }
