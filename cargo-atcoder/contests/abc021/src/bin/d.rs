...
fn main() {
    ...
    let mut factv: Vec<ModInt> = vec![ModInt::new(0); max + 1];
    let mut finvv: Vec<ModInt> = vec![ModInt::new(0); max + 1];
    factv[0] = ModInt::new(1);
    for i in 1..=max {
        factv[i] = factv[i - 1] * ModInt::new(i as u64);
    }
    finvv[max] = factv[max].inv();
    for i in (1..=max).rev() {
        finvv[i - 1] = finvv[i] * ModInt::new(i as i64);
    }

    let combination = |n: usize, r: usize| -> ModInt {
        if n < r {
            return ModInt::new(0);
        }
        factv[n] * finvv[r] * finvv[n - r]
    };

    let ans = combination(n - 1 + k, k);
    println!("{}", ans);
}
