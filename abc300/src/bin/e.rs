...
fn f(memo: &mut HashMap<usize, ModInt>, n: usize, i: usize) -> ModInt {
    if let Some(&v) = memo.get(&i) {
        return v;
    }
    match i.cmp(&n) {
        std::cmp::Ordering::Equal => ModInt::new(1),
        std::cmp::Ordering::Greater => ModInt::new(0),
