...
fn main() {
    ...
    let mut st = Segtree::<Min<usize>>::new(n);
    for (i, a_i) in a.iter().copied().enumerate().skip(k) {
        let v = *map.get(&a_i).unwrap();
        st.set(v, st.get(v).min(i));
    }
    let mut ans = n + 1;
    for (i, a_i) in a.iter().copied().enumerate().take(k) {
        let v = *map.get(&a_i).unwrap();
        let j = st.prod(v + 1, n);
    ...
    let ans = if ans > n { -1 } else { ans as i64 };
    println!("{}", ans);
}
